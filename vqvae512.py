    ...
    def __init__(self, in_channel, channel, n_res_block, n_res_channel, stride):
        ...
        if stride == 2:
            blocks = [
                ...
                nn.ReLU(inplace=True),
                nn.Conv2d(channel, channel, 3, padding=1),  # W // 2
            ]
...
class UpSample(nn.Module):
    def __init__(
        ...
        if stride == 4:
            blocks.extend(
                [
                    nn.ConvTranspose2d(in_channel, in_channel, 4, stride=2, padding=1),
                    nn.ReLU(inplace=True),
...
class VQVAE(nn.Module):
    def __init__(
        ...
        self.enc_b = Encoder(in_channel, channel, n_res_block, n_res_channel, stride=2)  # W // 2 C=channel
        self.enc_m = Encoder(channel, channel, n_res_block, n_res_channel, stride=2)  # W // 4 C=channel
        self.enc_t = Encoder(channel, channel, n_res_block, n_res_channel, stride=2)  # W // 8 C=channel
        # self.enc_ = Encoder(channel, channel, n_res_block, n_res_channel, stride=2)
        self.quantize_conv_t = nn.Conv2d(channel, embed_dim, 1)  # W // 8 C=embed_dim
        self.quantize_t = Quantize(embed_dim, n_embed)  # W // 8 C=embed_dim
        self.dec_t = Decoder(
            embed_dim, embed_dim, channel, n_res_block, n_res_channel, stride=2
        )  # W // 4 C=embed_dim
        self.quantize_conv_m = nn.Conv2d(embed_dim + channel, embed_dim, 1)  # W // 4 C=embed_dim
        self.quantize_m = Quantize(embed_dim, n_embed)  # W // 4 C=embed_dim

        self.dec_tmb = Decoder(
            embed_dim + channel, embed_dim + embed_dim, channel, n_res_block, n_res_channel, stride=2
        )
        self.dec_m = Decoder(
            embed_dim, embed_dim, channel, n_res_block, n_res_channel, stride=2
        )

        self.quantize_conv_b_1 = nn.Conv2d(channel * 2, embed_dim, 1)  # W // 2 C=embed_dim
        self.quantize_conv_b_2 = nn.Conv2d(channel, embed_dim, 1)  # W // 2 C=embed_dim
        self.quantize_b = Quantize(embed_dim, n_embed)  # W // 2 C=embed_dim

        self.upsample_m = UpSample(embed_dim, embed_dim, stride=2)
        self.upsample_t = UpSample(embed_dim, embed_dim, stride=4)  # W // 4 C=embed_dim
        self.dec = Decoder(
    ...
    def forward(self, input):
        quant_t, quant_m, quant_b, diff, _, _, _ = self.encode(input)
    ...
    def encode(self, input):
        ...
        enc_b = torch.cat([dec_tmb, enc_b], 1)
        quant_b = self.quantize_conv_b_1(enc_b)
        quant_b = torch.cat([dec_m, quant_b], 1)
        quant_b = self.quantize_conv_b_2(quant_b).permute(0, 2, 3, 1)
        quant_b, diff_b, id_b = self.quantize_b(quant_b)
        quant_b = quant_b.permute(0, 3, 1, 2)
        diff_b = diff_b.unsqueeze(0)

        return quant_t, quant_m, quant_b, diff_t + diff_m + diff_b, id_t, id_m, id_b
    ...
    def decode_code(self, code_t, code_m, code_b):
        quant_t = self.quantize_t.embed_code(code_t)
        quant_t = quant_t.permute(0, 3, 1, 2)
        quant_m = self.quantize_t.embed_code(code_m)
        quant_m = quant_m.permute(0, 3, 1, 2)
